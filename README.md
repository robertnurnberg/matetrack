...
options:
  ...
  --shortTBPVonly       for TB win/loss scores, only consider short PVs and wrong TB entries an issue (default: False)
...
The script `check_engine.sh` can be used to verify that an engine conforms to the expected behaviour in mate score announcements and the associated PVs. Run `./check_engine.sh --engine ./engine_name` to essentially run the Stockfish matetrack CI for the given engine, or alternatively run e.g. `./check_engine.sh --engine ./engine_name --nodes 100 --time 0 --goMateNodes 0` for a much quicker check.
